...
class RappiBLoC with ChangeNotifier{
  ...
  void init(TickerProvider ticker){
    ...
    for (int i =0; i< rappiCategories.length ; i++){
      ...
      if(i < rappiCategories.length - 1){//Se trae el ofset del que sigue
        offsetTo = offsetFrom + rappiCategories[i + 1].products.length.toDouble() * productHeight ; 
  ...
  void _onScrollListener(){//para el SELECCIONAR el Tab horizontal en base al scroll horizontal
    if(_listen){//Si tienes que escuchar, haz esto
      for(int i = 0; i < tabs.length; i++){
        final tab = tabs[i]; //El tab actual
        if ( (scrollController.offset >= tab.offsetFrom)  &&  
            (scrollController.offset <= tab.offsetTo) && //Si estoy en cierta categoria vertical
            ( !tab.selected)  )  { // de la cual no tengo seleccionado su tab
        
            onCategorySelected(i, animationRequired: false);  //le paso la categoria en la que esta para que se acomode solo
            tabController.animateTo(i); //Para que lo podamos ver si estamos en un tab lejano
  ...
  void onCategorySelected(int index, {bool animationRequired = true} ) async {
    ...
    if( animationRequired){
      _listen = false; //se bloquea esta animacion para no hacer un glich con 2 en el horizontal
  ...
  @override
  void dispose() {
    scrollController.removeListener( _onScrollListener); //Para el Scroll automatico horizontal de tabs
